...
export default function Programmers() {
  ...
  const [ programmers, newProgrammers ] = useState(listOfAwesome);
  const [ featuredProgrammer, newFeatured ] = useState();
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers.find(dev => dev.id === featuredProgrammer).name;
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {newFeatured(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
